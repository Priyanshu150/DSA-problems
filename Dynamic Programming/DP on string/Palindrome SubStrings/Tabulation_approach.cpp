...
// the idea is if string 'a' or 'aa' is palindrom, then if we add a eqaul char at both side then it also be a palindrome 'cac' or 'caac'
...
// which we can check via dp[i+1][j-1] is true, then dp[i][j] will also be true i.e current length will increase by 2
