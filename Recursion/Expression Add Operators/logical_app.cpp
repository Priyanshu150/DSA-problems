...
//Time complextiy :- O(4^n * n)
// Space complexity :- O(n)(recursion stack)+O(4^n * n) (output)​

// Approach :-  
// we'll use recursion and backtracking to explore all the functionallity 
// parallely we've to caluclate the sum of the expression 
// f(ind, s, target, res, temp_str, prev, sum) will be our parameter for function
// prev will be the previous value for number we caluclate, this will be use for '*'
// for +, sum = sum+curr and prev = curr
// for -, sum = sum-curr and prev = -curr
// for *, sum = sum - prev + prev*curr, prev = prev*curr, due to more precedency of '*' 
// use for loop to take 2-3 and so on digit number 
