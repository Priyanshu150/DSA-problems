...
// Approach :-  
// use min_heap to store number in format {lastNumberInSequence, lengthOfSequence}
// for every number, check if the top element of heap 
// if it's equal, create a new sequence 
// if it's one more than top element, add that number in this sequence 
// it the current number is greater, then check if the top sequence has lenght greater than k or not 
// if not then it's not possible otherwise pop it from the array 
// at last check all the sequence in the array has a length greater than k or not 
